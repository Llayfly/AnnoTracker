...
自动从数据标注平台 `data-platform.synapath.com` 采集标注员工作数据。页面按组织展示 蓝=新任务 / 黄=旧任务 双色柱状图（各自显示时长数值）：

- **历史数据（2026-08-22 及之前）**：来自 `daily_stats` 表，原始时长 = 新任务 + 旧任务，柱状图 蓝=新任务 / 黄=旧任务。
- **新数据（2026-08-23 起）**：每天 23:59（北京时间）定时快照的单日数据，柱状图 蓝=今日提交新增 / 黄=今日修改旧任务。
...
- 旧任务（今日修改）、片段时长、无片段、PASS 占比、结算参考等数据照常采集与记录
...
- **标注员搜索**：按名称实时模糊搜索
- **分组柱状图**：按组织分组（HC / C / HBHC / S / JS / 其他），每组为 蓝=新任务 / 黄=旧任务 双色柱状图，各自显示时长数值
- **每日明细**：点击柱状图上的标注员查看逐日数据（含来源：自动/手动）
