...
自动从数据标注平台 `data-platform.synapath.com` 采集标注员工作数据。页面分两个区块展示，互不混淆：

- **历史数据（2026-08-22 及之前）**：来自 `daily_stats` 表，原始时长 = 新任务 + 旧任务，含累计参考、柱状图、分组织。
- **新数据（2026-08-23 起）**：每天 23:59（北京时间）定时快照的单日数据，原始时长 = 新任务之和，含柱状图、分组织。
