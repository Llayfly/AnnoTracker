'use strict';
// 数据采集模块 —— Vercel serverless 版（异步 Turso）
const axios = require('axios');
const { db, ensureInit, ensureAnnotator, logRun } = require('./db');
...
async function collectDay(dateStr) {
  ...
  for (const row of rows) {
    const annotatorId = await ensureAnnotator(row.person_label);
  ...
  console.log(`[collector] ${dateStr} 采集完成，${count} 名标注员`);
  return count;
}
...
async function collectCumulative(endDateStr) {
  ...
  console.log(`[collector] 累计快照采集完成，${count} 名标注员`);
  return count;
}
...
module.exports = {
  collectDay, collectCumulative, collectToday, collectYesterday,
  ensureInit, fmtDate,
};
