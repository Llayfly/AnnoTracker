...
// 带重试的 GET 请求：处理瞬时网络抖动/限流（最多2次，短退避；避免在平台不可达时拖满60s导致504）
async function authGet(path, params, retries = 2) {
  let lastErr;
...
async function collectSnapshotLatest() {
  ...
  const deadline = Date.now() + 50000; // 50s 硬预算，避免超出 Vercel 60s 上限
  const results = [];
