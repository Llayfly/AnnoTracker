...
async function collectBatches() {
  ...
  const allBatches = [];
  let page = 1;
  const pageSize = 50; // 平台限制每页最多50条
  const maxPages = 20; // 安全上限：最多20页 = 1000条
  let total = 0;
  ...
  while (page <= maxPages) {
    ...
    total = pagination.total || allBatches.length + batches.length;
    allBatches.push(...batches);
    console.log(`[collector] 批次采集 page ${page}: ${batches.length} 条, total=${total}`);
    if (batches.length === 0 || allBatches.length >= total) break;
    page++;
  }
  ...
  // 2. 删除旧的自动采集记录（保留手动添加的记录）
  await db.execute({
    sql: "DELETE FROM batches WHERE note LIKE '[AUTO]%'",
    args: [],
  });
  ...
  const insertStmts = allBatches.map((b) => {
    ...
    const note = `[AUTO] ${b.assignment_note || ''} ${b.decision_note || ''}`.trim();
