...
// 采集最新可用的单日快照（23:59 定时调用）
// 平台每天 00:23 生成前一天数据，因此 23:59 时当天数据尚未生成，回退采集前一天
// 仅采集 2026-08-23 及之后的数据（与旧数据完全分离）
async function collectSnapshotLatest() {
  ...
  try {
    ...
    if (count > 0) {
      ...
      return { date: todayStr, count, mode: 'today' };
    }
  } catch (e) {
    console.error('[collector] 快照采集当天失败:', e.message);
  }

  // 2) 当天无数据，回退采集前一天（且不早于 2026-08-23）
  const d = new Date();
  d.setDate(d.getDate() - 1);
  const yesterdayStr = fmtDate(d);
  if (yesterdayStr < SNAPSHOT_START_DATE) {
    await logRun('snapshot', todayStr, 'info', '尚未到可采集日期（数据未生成）', 0);
    return { date: null, count: 0, mode: 'none' };
  }
  const count = await collectSnapshotDay(yesterdayStr);
  await logRun('snapshot', yesterdayStr, 'success', '快照采集完成（前一天）', count);
  return { date: yesterdayStr, count, mode: 'yesterday' };
}
