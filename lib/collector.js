...
const config = {
  ...
  // 支持多组织采集，逗号分隔
  orgs: (process.env.AM_PLATFORM_ORGS || 'HC,C,S').split(',').map((s) => s.trim()).filter(Boolean),
  ...
  for (const org of config.orgs) {
    try {
      ...
      if (rows.length) {
        ...
        console.log(`[collector] ${dateStr} [${org}] 采集到 ${rows.length} 人`);
      } else {
        console.log(`[collector] ${dateStr} [${org}] 无数据`);
      }
    } catch (e) {
...
async function collectBatches() {
  ...
  const limit = 50;
  const maxRequests = 25; // 安全上限：每个组织最多25次请求
