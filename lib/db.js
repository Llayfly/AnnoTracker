...
const INIT_STATEMENTS = [
  ...
  `CREATE INDEX IF NOT EXISTS idx_batches_project ON batches(project_type)`,
];
...
async function ensureInit() {
  ...
  _initPromise = (async () => {
    try {
      ...
      await ensureBatchColumns(client);
      _initialized = true;
...
// 批次表增量加列（兼容已建库的旧表结构，幂等：已存在则跳过）
async function ensureBatchColumns(client) {
  try {
    const cols = await client.execute({ sql: 'PRAGMA table_info(batches)', args: [] });
    const names = new Set((cols.rows || []).map((c) => c.name));
    const adds = [];
    if (!names.has('task_combination')) adds.push('ALTER TABLE batches ADD COLUMN task_combination TEXT');
    if (!names.has('platform_created_at')) adds.push('ALTER TABLE batches ADD COLUMN platform_created_at TEXT');
    for (const sql of adds) {
      await client.execute({ sql, args: [] });
    }
    if (adds.length) console.log('[db] batches 表已补充新列:', adds.join(' | '));
  } catch (e) {
    // 列已存在或平台不支持时忽略，不影响主流程
    console.error('[db] ensureBatchColumns 跳过:', e.message);
  }
}
