...
async function migrateLabels() {
  ...
  const allLabels = await db.execute({ sql: 'SELECT id, label FROM annotators' });
  for (const row of allLabels.rows) {
    const lbl = row.label;
    const upper = lbl.toUpperCase();
    if (lbl === upper) continue;
    const targetRes = await db.execute({ sql: 'SELECT id FROM annotators WHERE label = ? AND id != ?', args: [upper, row.id] });
    if (targetRes.rows.length) {
      const newId = targetRes.rows[0].id;
      await db.batch([
        { sql: 'UPDATE daily_stats SET annotator_id = ? WHERE annotator_id = ?', args: [newId, row.id] },
        { sql: 'UPDATE annotator_cumulative SET annotator_id = ? WHERE annotator_id = ?', args: [newId, row.id] },
        { sql: 'DELETE FROM annotators WHERE id = ?', args: [row.id] },
      ], 'write');
      results.push({ old: lbl, new: upper, status: 'case-merged', oldId: row.id, newId });
    } else {
      await db.execute({ sql: 'UPDATE annotators SET label = ? WHERE id = ?', args: [upper, row.id] });
      results.push({ old: lbl, new: upper, status: 'case-renamed', oldId: row.id });
    }
  }
  return results;
}
